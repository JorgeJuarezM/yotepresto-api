# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# Note that this schema.rb definition is the authoritative source for your
# database schema. If you need to create the application database on another
# system, you should be using db:schema:load, not running all the migrations
# from scratch. The latter is a flawed and unsustainable approach (the more migrations
# you'll amass, the slower it'll run and the greater likelihood for issues).
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema.define(version: 20190925182051) do

  create_table "account_types", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
  end

  create_table "addresses", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "address"
    t.bigint "municipality_id"
    t.string "postal_code"
    t.string "phone"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["municipality_id"], name: "index_addresses_on_municipality_id"
  end

  create_table "bank_accounts", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "number"
    t.decimal "opening_balance", precision: 10
    t.bigint "user_id"
    t.bigint "bank_branch_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.bigint "account_type_id"
    t.string "clabe"
    t.decimal "balance", precision: 10
    t.index ["account_type_id"], name: "index_bank_accounts_on_account_type_id"
    t.index ["bank_branch_id"], name: "index_bank_accounts_on_bank_branch_id"
    t.index ["user_id"], name: "index_bank_accounts_on_user_id"
  end

  create_table "bank_branches", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.bigint "bank_id"
    t.bigint "location_id"
    t.string "address"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["bank_id"], name: "index_bank_branches_on_bank_id"
    t.index ["location_id"], name: "index_bank_branches_on_locations_id"
  end

  create_table "banks", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.string "abbrv"
    t.string "abm"
    t.string "code"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
  end

  create_table "countries", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.string "abbrv"
    t.string "alfa_2"
    t.string "alfa_3"
    t.string "numeric_code"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
  end

  create_table "locations", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.string "code"
    t.string "abbrv"
    t.bigint "municipality_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.string "location_code"
    t.index ["municipality_id"], name: "index_locations_on_municipality_id"
  end

  create_table "municipalities", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.string "code"
    t.string "abbrv"
    t.bigint "state_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["state_id"], name: "index_municipalities_on_states_id"
  end

  create_table "roles", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.string "resource_type"
    t.bigint "resource_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["name", "resource_type", "resource_id"], name: "index_roles_on_name_and_resource_type_and_resource_id"
    t.index ["name"], name: "index_roles_on_name"
    t.index ["resource_type", "resource_id"], name: "index_roles_on_resource_type_and_resource_id"
  end

  create_table "states", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.string "code"
    t.bigint "country_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.string "abbrv"
    t.index ["country_id"], name: "index_states_on_country_id"
  end

  create_table "transaction_histories", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.bigint "by_user_id"
    t.bigint "transaction_type_id"
    t.decimal "amount", precision: 10
    t.bigint "account_from_id"
    t.bigint "account_to_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["account_from_id"], name: "index_transaction_histories_on_account_from_id"
    t.index ["account_to_id"], name: "index_transaction_histories_on_account_to_id"
    t.index ["by_user_id"], name: "index_transaction_histories_on_by_user_id"
    t.index ["transaction_type_id"], name: "index_transaction_histories_on_transaction_type_id"
  end

  create_table "transaction_types", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
  end

  create_table "users", force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.string "name"
    t.string "username"
    t.string "email"
    t.string "password_digest"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.string "key"
    t.string "last_name"
    t.datetime "dob"
    t.string "id_card"
  end

  create_table "users_roles", id: false, force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=utf8" do |t|
    t.bigint "user_id"
    t.bigint "role_id"
    t.index ["role_id"], name: "index_users_roles_on_role_id"
    t.index ["user_id", "role_id"], name: "index_users_roles_on_user_id_and_role_id"
    t.index ["user_id"], name: "index_users_roles_on_user_id"
  end

  add_foreign_key "addresses", "municipalities"
  add_foreign_key "bank_accounts", "account_types"
  add_foreign_key "bank_accounts", "bank_branches"
  add_foreign_key "bank_accounts", "users"
  add_foreign_key "bank_branches", "banks"
  add_foreign_key "bank_branches", "locations"
  add_foreign_key "locations", "municipalities"
  add_foreign_key "municipalities", "states"
  add_foreign_key "states", "countries"
  add_foreign_key "transaction_histories", "bank_accounts", column: "account_from_id"
  add_foreign_key "transaction_histories", "bank_accounts", column: "account_to_id"
  add_foreign_key "transaction_histories", "transaction_types"
  add_foreign_key "transaction_histories", "users", column: "by_user_id"
end
